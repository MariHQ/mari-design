...
import type { Task, TasksData } from "../../pages/TasksPage";
...
const board = (tasks: Task[]): TasksData => ({ tasks, draft: "", saving: false, strip: null });

const DEFAULT = board(SEED);

/** A brand-new inbox. Genuinely no rows, so the page's own per-column empty
    text fires — the canvas is not faking the empty state. */
const EMPTY = board([]);
