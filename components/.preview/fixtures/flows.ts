...
export const FIXTURES: PageFixtures<FlowsData> = {
  ...
  "trigger-manual": { data: view({ trigger: { kind: "manual", flow: "Onboarding checker", on: "" } }) },
  "trigger-schedule": { data: view({ trigger: { kind: "schedule", flow: "Slack digest", on: "schedule" } }) },
  "trigger-document": { data: view({ trigger: { kind: "document", flow: "Docs guardrail", on: "document_changed" } }) },
