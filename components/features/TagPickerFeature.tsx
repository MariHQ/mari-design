      ...
      <div className={`${card} overflow-hidden`}>
        {rows.map((d, i) => (
          <div key={d.id} className={`flex items-start gap-3 px-4 py-3.5 ${i > 0 ? "border-t border-ink/10" : ""}`}>
            <span className="mt-0.5 shrink-0 text-ink/40"><FileText size={16} /></span>
            <div className="min-w-0 flex-1">
              <div className="text-[14px] font-medium text-ink">{d.title}</div>
              <div className="mt-0.5 flex items-center gap-2">
                <SourceMark provider={d.provider} size={13} />
                <span className="font-term text-[11px] text-ink/50">{d.source}</span>
                <span className="font-term text-[11px] text-ink/40">· updated {fmtAgo(d.updatedAt)}</span>
              </div>
              <div className="mt-2 flex flex-wrap items-center gap-1.5">
                {d.tags.length > 0 ? (
                  d.tags.map((t) => <TagChip key={t} tag={t} />)
                ) : (
                  <span className="font-term text-[11px] text-ink/40">No tags</span>
                )}
              </div>
            </div>
