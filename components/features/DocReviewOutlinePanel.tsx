...
const DEMO_REVS: Rev[] = [
  { id: 1, actor: "Aki Kim", verb: "edited Overview", at: "2h ago" },
  { id: 2, actor: "Lena Shah", verb: "ran Tighten", at: "Yesterday, 4:12 PM" },
  { id: 3, actor: "Maya Chen", verb: "accepted 3 changes", at: "Jul 18, 11:03 AM" },
  { id: 4, actor: "Aki Kim", verb: "ran fact check", at: "Jul 17, 9:40 AM" },
  { id: 5, actor: "Sam Ortiz", verb: "created the document", at: "Jul 15, 2:15 PM" },
  { id: 6, actor: "Lena Shah", verb: "imported from docs", at: "Jul 15, 2:14 PM" },
];
      ...
      <Card title="Document outline">
        <div className="space-y-0.5">
          {outline.map((o) => (
            ...
            >
              {!o.sub && <span className="inline-block w-1.5 h-1.5 shrink-0 rounded-full bg-clay" />}
              {o.n && <span className="font-term text-[11px] text-ink/45">{o.n}</span>}
              <span className="min-w-0 truncate">{o.t}</span>
            </button>
          ))}
          {outline.length === 0 && (
            <div className="text-[12.5px] text-ink/45">No headings yet — add a section with the H1 menu.</div>
          )}
        </div>
      </Card>
            ...
            <div key={r.id} className="flex items-start gap-2.5">
              ...
              <span className="min-w-0 flex-1">
                <span className="flex items-center gap-1.5 text-[13px] text-ink">
                  {r.actor}
                  {i === 0 && <Chip label="Current" tone="ok" dot pulse />}
                </span>
                <span className="mt-0.5 block text-[11px] text-ink/50">{r.verb} · {r.at}</span>
              </span>
            </div>
          ))}
        </div>
