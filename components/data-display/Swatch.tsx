...
export function Swatch({
  ...
  const shared = `inline-flex items-center gap-2 rounded-[4px] border px-2 py-1.5 ${selected ? "border-biscay-2 ring-1 ring-biscay-2/40" : "border-ink/15"} ${className}`.trim();
  const body = (
    <>
      <span className="w-4 h-4 rounded-[3px] border border-ink/15 shrink-0" style={{ background: color }} aria-hidden="true" />
      {label && <span className="font-term text-[12px] text-ink/70">{label}</span>}
    </>
  );
  if (onClick) {
    return (
