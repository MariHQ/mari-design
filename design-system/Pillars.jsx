...
function Pillars() {
  const items = [
    { name: "Knowledge",  hue: "primary", icon: "icon-mountain-spirit.svg", desc: "Continuously ingests Slack threads, PRs, and architecture diagrams into a single source of truth." },
    { name: "Experience", hue: "fg",      icon: "icon-cave-guardian.svg",   desc: "Captures the hard-won tribal context that engineers carry in their heads." },
    { name: "Intuition",  hue: "primary", icon: "icon-flame-spark.svg",     desc: "Style-tuned LLMs draft prose that sounds like your senior staff wrote it." },
    { name: "Scale",      hue: "fg",      icon: "icon-storm-cloud.svg",     desc: "Detects stale docs before your on-call engineer learns the hard way." },
    { name: "Automation", hue: "rose",    icon: "icon-golden-thread.svg",   desc: "Ships pull requests — not Notion tickets. Humans review, machines toil." },
  ];
        ...
        <div style={{
          ...
          {items.map((p, i) => (
            <div key={p.name} style={{ background: "hsl(var(--card))", padding: 24 }}>
              <div style={{
                ...
                background: "hsl(var(--surface-3, 30 18% 86%))",
              }}>
                <img src={`../../assets/${p.icon}`} alt="" width="44" height="44" style={{ display: "block" }} />
              </div>
          ...
          <div style={{ background: "hsl(var(--card))", padding: 24, position: "relative", overflow: "hidden",
            ...
            <div style={{ position: "relative" }}>
              <img src="../../assets/icon-tree-of-mari.svg" alt="" width="56" height="56" style={{ display: "block", marginBottom: 12 }} />
